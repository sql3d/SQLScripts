SELECT 
    ...
    ELSE 
        CASE is_primary_key WHEN 1 THEN
            N'ALTER TABLE ' + sc.name + N'.' + t.name + N' ADD CONSTRAINT ' + si.name + N' PRIMARY KEY ' +
                CASE WHEN si.index_id > 1 THEN N'NON' ELSE N'' END + N'CLUSTERED '
            ELSE N'CREATE ' + 
                CASE WHEN si.is_unique = 1 then N'UNIQUE ' ELSE N'' END +
                CASE WHEN si.index_id > 1 THEN N'NON' ELSE N'' END + N'CLUSTERED ' +
                N'INDEX ' + si.name + N' ON ' + sc.name + N'.' + t.name + N' '
        END +
        ...
        CASE WHEN row_compression_partition_list IS NOT NULL OR page_compression_partition_list IS NOT NULL 
            THEN N' WITH (' +
                CASE WHEN row_compression_partition_list IS NOT NULL THEN
                    N'DATA_COMPRESSION = ROW ON PARTITIONS (' + row_compression_partition_list + N')'
                ELSE N'' END +
                ...
                CASE WHEN page_compression_partition_list IS NOT NULL THEN
                    N'DATA_COMPRESSION = PAGE ON PARTITIONS (' + page_compression_partition_list + N')'
                ELSE N'' END
            + N')'
            ELSE N''
        END +
...
LEFT JOIN sys.dm_db_index_usage_stats AS stat ON si.object_id=stat.object_id and si.index_id=stat.index_id
...
/* Partitioning Ordinal */ OUTER APPLY (
    SELECT MAX(c.name) AS column_name
    FROM sys.index_columns AS ic 
    JOIN sys.columns AS c ON 
        ic.column_id=c.column_id  
        and ic.object_id=c.object_id
